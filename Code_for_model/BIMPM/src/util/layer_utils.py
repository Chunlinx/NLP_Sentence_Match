...
def calcuate_attention( in_value_1, in_value_2, feature_dim1, feature_dim2, 
                        scope_name='att', att_type='symmetric', att_dim=20, 
                        remove_diagnoal=False, mask1=None, mask2=None, is_training=False, dropout_rate=0.2):
    ...
    with tf.variable_scope(scope_name):
        ...
        # 相当于 对每一个 feature_dim 特征 做了一个矩阵变换，映射到 att_dim 的向量中；
        ...
        if att_type == 'additive':
            atten_b = tf.get_variable("atten_b", [att_dim], dtype=tf.float32) # [ att_dim ]
            atten_v = tf.get_variable("atten_v", [1, att_dim], dtype=tf.float32) # [ 1, att_dim ]
