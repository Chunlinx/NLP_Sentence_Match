...
def cosine_distance(y1,y2):
    # y1 [....,a, 1, d]
    # y2 [....,1, b, d]
    cosine_numerator = tf.reduce_sum( tf.multiply(y1, y2), axis=-1 ) # [batch_size, passage_len, question_len, dim] -> [batch_size, passage_len, question_len]
...
def multi_perspective_match(feature_dim, repres1, repres2, is_training=True, dropout_rate=0.2,
                            config=None, scope_name='mp-match', reuse=False):
    '''
    ...
    with tf.variable_scope(scope_name, reuse=reuse):
        ...
        if config['model']['with_mp_cosine']:
            mp_cosine_params = tf.get_variable("mp_cosine", shape=[config['model']['cosine_MP_dim'], feature_dim], dtype=tf.float32)
            ...
            match_dim += config['model']['cosine_MP_dim']
...
def match_passage_with_question(passage_reps, question_reps, passage_mask, question_mask, passage_lengths, question_lengths,
                                ...
                                dropout_rate=0, config=None, is_training=True, forward=True):
    ...
    with tf.variable_scope(scope or "match_passage_with_question"):
        ...
        #             scope_name="fw_attention", att_type=config.att_type, att_dim=config.att_dim,
        ...
        if with_full_match:
            ...
            (attentive_rep, match_dim) = multi_perspective_match(context_lstm_dim,
                                passage_reps, question_full_rep, is_training=is_training, dropout_rate=config['model']['dropout_rate'],
                                config=config, scope_name='mp-match-full-match')
        ...
        if with_maxpool_match:
            maxpooling_decomp_params = tf.get_variable("maxpooling_matching_decomp",
                                                          shape=[ config['model']['cosine_MP_dim'], context_lstm_dim], dtype=tf.float32)
            ...
            dim += 2*config['model']['cosine_MP_dim']

        if with_attentive_match:
            atten_scores = layer_utils.calcuate_attention(passage_reps, question_reps, context_lstm_dim, context_lstm_dim,
                    scope_name="attention", att_type=config['model']['att_type'], att_dim=config['model']['att_dim'],
                    remove_diagnoal=False, mask1=passage_mask, mask2=question_mask, is_training=is_training, dropout_rate=dropout_rate)
            att_question_contexts = tf.matmul(atten_scores, question_reps)
            (attentive_rep, match_dim) = multi_perspective_match(context_lstm_dim,
                    passage_reps, att_question_contexts, is_training=is_training, dropout_rate=config['model']['dropout_rate'],
                    config=config, scope_name='mp-match-att_question')
            all_question_aware_representatins.append(attentive_rep)
            dim += match_dim
        ...
        if with_max_attentive_match:
            ...
            (max_attentive_rep, match_dim) = multi_perspective_match(context_lstm_dim,
                    passage_reps, max_att, is_training=is_training, dropout_rate=config['model']['dropout_rate'],
                    config=config, scope_name='mp-match-max-att')
...
def bilateral_match_func(in_question_repres, in_passage_repres,
                        question_lengths, passage_lengths, question_mask, passage_mask, input_dim, is_training, config=None):
    ...
    (match_reps, match_dim) = match_passage_with_question(in_passage_repres, in_question_repres, passage_mask, question_mask, passage_lengths,
                                ...
                                with_full_match=config['model']['with_full_match'],
                                with_maxpool_match=config['model']['with_maxpool_match'],
                                with_attentive_match=config['model']['with_attentive_match'],
                                with_max_attentive_match=config['model']['with_max_attentive_match'],
                                dropout_rate=config['model']['dropout_rate'],
                                config=config, is_training=is_training, forward=False)
    ...
    with tf.variable_scope('context_MP_matching'):
        for i in range(config['model']['context_layer_num'] ): # support multiple context layer
            with tf.variable_scope('layer-{}'.format(i)):
                ...
                (question_context_representation_fw, question_context_representation_bw,
                 in_question_repres) = layer_utils.my_lstm_layer(
                        in_question_repres, config['model']['context_lstm_dim'], input_lengths=question_lengths, scope_name="context_represent",
                        reuse=False, is_training=is_training, dropout_rate=config['model']['dropout_rate'], use_cudnn=config['model']['use_cudnn'] )
                
                (passage_context_representation_fw, passage_context_representation_bw, 
                 in_passage_repres) = layer_utils.my_lstm_layer(
                        in_passage_repres, config['model']['context_lstm_dim'], input_lengths=passage_lengths, scope_name="context_represent",
                        reuse=True, is_training=is_training, dropout_rate=config['model']['dropout_rate'], use_cudnn=config['model']['use_cudnn'] )
                ...
                with tf.variable_scope('left_MP_matching'):
                    (match_reps, match_dim) = match_passage_with_question(passage_context_representation_fw,
                                ...
                                question_lengths, config['model']['context_lstm_dim'], scope="forward_match",
                                with_full_match=config['model']['with_full_match'], 
                                with_maxpool_match=config['model']['with_maxpool_match'],
                                with_attentive_match=config['model']['with_attentive_match'],
                                with_max_attentive_match=config['model']['with_max_attentive_match'],
                                is_training=is_training, config=config, dropout_rate=config['model']['dropout_rate'], forward=True)
                    question_aware_representatins.append(match_reps)
                    question_aware_dim += match_dim
                    (match_reps, match_dim) = match_passage_with_question(passage_context_representation_bw,
                                ...
                                question_lengths, config['model']['context_lstm_dim'], scope="backward_match",
                                with_full_match=config['model']['with_full_match'], 
                                with_maxpool_match=config['model']['with_maxpool_match'],
                                with_attentive_match=config['model']['with_attentive_match'],
                                with_max_attentive_match=config['model']['with_max_attentive_match'],
                                is_training=is_training, config=config, dropout_rate=config['model']['dropout_rate'], forward=False)
                    question_aware_representatins.append(match_reps)
                    question_aware_dim += match_dim
                ...
                with tf.variable_scope('right_MP_matching'):
                    (match_reps, match_dim) = match_passage_with_question(question_context_representation_fw,
                                ...
                                passage_lengths, config['model']['context_lstm_dim'], scope="forward_match",
                                with_full_match=config['model']['with_full_match'], 
                                with_maxpool_match=config['model']['with_maxpool_match'],
                                with_attentive_match=config['model']['with_attentive_match'],
                                with_max_attentive_match=config['model']['with_max_attentive_match'],
                    ...
                    (match_reps, match_dim) = match_passage_with_question(question_context_representation_bw,
                                ...
                                passage_lengths, config['model']['context_lstm_dim'], scope="backward_match",
                                with_full_match=config['model']['with_full_match'], 
                                with_maxpool_match=config['model']['with_maxpool_match'],
                                with_attentive_match=config['model']['with_attentive_match'],
                                with_max_attentive_match=config['model']['with_max_attentive_match'],
    ...
    if is_training is not None:
        question_aware_representatins = tf.nn.dropout(question_aware_representatins, (1 - config['model']['dropout_rate'] ))
        passage_aware_representatins = tf.nn.dropout(passage_aware_representatins, (1 - config['model']['dropout_rate'] ))
        
    # ======Highway layer======
    if config['model']['with_match_highway']:
        with tf.variable_scope("left_matching_highway"):
            question_aware_representatins = multi_highway_layer(question_aware_representatins, question_aware_dim,
                                                                config['model']['highway_layer_num'] )
    ...
    with tf.variable_scope('aggregation_layer'):
        for i in range( config['model']['aggregation_layer_num'] ): # support multiple aggregation layer
            ...
            (fw_rep, bw_rep, cur_aggregation_representation) = layer_utils.my_lstm_layer(
                        qa_aggregation_input, config['model']['aggregation_lstm_dim'], input_lengths=passage_lengths, scope_name='left_layer-{}'.format(i),
                        reuse=False, is_training=is_training, dropout_rate=config['model']['dropout_rate'], use_cudnn=config['model']['use_cudnn'] )
            ...
            aggregation_dim += 2* config['model']['aggregation_lstm_dim']
            ...
            (fw_rep, bw_rep, cur_aggregation_representation) = layer_utils.my_lstm_layer(
                        pa_aggregation_input, config['model']['aggregation_lstm_dim'],
    ...
    # ======Highway layer======
    if config['model']['with_aggregation_highway']:
        with tf.variable_scope("aggregation_highway"):
            ...
            aggregation_representation = multi_highway_layer(aggregation_representation, aggregation_dim, config['model']['highway_layer_num'] )
